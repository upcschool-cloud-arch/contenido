...
13. Vamos a intentar dejar todo en la Main_Route_Table. En el dashboard de VPC, y selecciona la Main_Route_Table, que será la route table asociada a vuestra VPC main_vpc_yourname
14. Clicamos sobre la pestaña _Routes_ y añadimos la línea del Internet Gateway, definiendo el como destination: 0.0.0.0/0 y el Target: el ID del Internet Gateway creado en el lab 1.
...
16. Vamos a ver qué ocurre si dejamos únicamente el nat gateway. Volvemos al apartado de _Route_Tables y seleccionamos la Custom_main_route_table que es dónde tenemos definida nuestra salida a internet. 
17. Clicamos sobre subnet associations y en _Edit_subnet_associations_ y eliminamos la asociación explícita, quitando el check de la main_subnet_a.
18. Vamos a probar llegar desde internet hasta la EC2. Para ello haz ping a la nueva IP de instancia EC2 del lab1 (aseguraros de que está running). ¿Cuál es el resultado?
...
19. Accede a través de la shell de la instancia a la EC2. Para ello, clica en la consola, arriba a la derecha sobre el botón _Connect_
20. El resultado es que tampoco podemos conectarnos. Asocia de nuevo **explícitamente** la Custom_main_route_Table a la main_subnet_a
...
21. Crearemos una EC2 de características muy similares a lab1 pero sin públic IP
22. Desde el dashboard de EC2, creamos esta EC2:
...
23. En el dashboard de EC2, buscamos la instancia lab1 y la iniciamos a través del botón _Instance State_--> _Start Instances_ en caso de estar parada.
24. Copiamos la IP pública que se muestra en la pestaña _Details_
25. Accedemos por ssh a la instancia lab1. Abrimos el terminal y utilizamos el siguiente comando si trabajamos con Linux:
...
26. Una vez dentro de la instancia de salto, podemos acceder a la instancia privada _lab2_. Pero antes, tendremos que realizar algunas configuraciones previas.
27. En primer lugar, tendremos que crear un fichero copiando el text de lab1.pem. y guardándolo con este mismo nombre. Recordad poneros en root para evitar problemas de permisos.
28. Un vez tenemos el fichero creado, cambiaremos los permisos:
```bash
chmod 400 lab1.pem
```

29. Desde el mismo directorio dónde hemos creado este fichero .pem accedemos a la instancia con el siguiente comando:
 ```bash
...
```bash
 ping 8.8.8.8 // Google
 ```
31. Si lanzamos un _traceroute 8.8.8.8_, podemos ver el camino que sigue nuestro paquete IP y si realmente está pasando por el Nat Gateway. Podéis verificar la IP (privada) de vuestro Nat Gateway en la consola de AWS.

## Peering Connection

En esta segunda parte del lab, vamos a establecer una conexión por peering entre nuestras VPCs

32. A través de la consola, accede al dashboard de VPCs. En las opciones de la izquierda, encontraréis un apartado llamado _Peering Connections_ .
33. Clicad sobre el botón _Create peering connection_
34. Nombraremos a este peering my_first_peering_connection y seleccionaremos nuestra main_vpc_yourname como VPC requester
35. Seleccionamos _my account_ y _This region_, en VPC accepter selecciona la VPC default y clicad sobre el botón _Create peering connection_.
36. ¿Qué ha ocurrido?
...
37. Seleccionad como VPC Accepter la secondary_vpc (creada en el _Bonus track_ del lab anterior). 
38. Añadimos el tag key: lab y value: 2 y clicamos sobre el boton _create peering connection_
39. Como owners de la secondary VPC, nos llegará una notificación de aceptación del peering. La aceptamos y ya tendremos el peering creado.
40. Si queremos establecer comunicación, deberemos crear las rutas correspondientes tanto en la vpc orígen, como destino.
41. Accedemos al listado de route tables y clicamos sobre la main route table de la main_vpc_your_name. Agregamos una nueva ruta con destino: 10.0.0.0/24 a través del target peering_id.
42. En la main route table de la secondary_vpc agregamos una nueva ruta con destino 172.31.0.0/16 a través del target peering_id.

Vamos a verificar que la comunicación entre instancias de estas VPCS funciona correctamente.

43. En la VPC secondary, creamos una EC2 llamada lab2_secondary, en la que **NO** tendremos IP pública.
* Name: lab2_secondary
...
44. Un vez creada esta instancia, podemos hacer ping desde la ec2 lab2 hacia la ip privada de lab2_secondary
...
45. A través del VPC dashboard, accedemos al apartado NAT Gateways y clicamos en el único que tenemos creado.
46. Clicamos sobre el botón _Actions_ y seleccionamos Delete NAT Gateway. Escribimos delete en el cuadro y finalizamos.
